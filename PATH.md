...
- 프로젝트 구조 schema version은 2로 유지하고 manager 갱신 여부는
  `manager_version`으로 관리
- `.codex-project.json`의 구조 번호 키를 `version`에서 `schema_version`으로
  명확화
...
클라우드 스토리지 설치 가이드와 레거시 전환 정리를 개선한 버전입니다.

- 클라우드 스토리지 설치용 가이드 `CLOUD_STORAGE_INSTALL.md` 추가
- README 상단에 클라우드 스토리지 설치 가이드 링크 추가
- 설정 파일과 프로젝트 메타데이터에 기록되는 manager 버전을 V4.1로 갱신
